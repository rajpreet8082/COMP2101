...
hostName=$(hostname)
lanA=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
labHN=$(getent hosts $lanA | awk '{print $2}')
externalA=$(curl -s icanhazip.com)
externalHN=$(getent hosts $externalA | awk '{print $2}')
routA=$(ip route | awk '{print $3;exit}')
routN=$(getent hosts $routA | awk '{print $2}')

cat <<EOF
Hostname        : $hostName
LAN Address     : $lanA
LAN Hostname    : $labHN
External IP     : $externalA
External Name   : $externalHN
Router Address  : $routA
Router Hostname : $routN
EOF
